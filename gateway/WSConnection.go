...
func (wsConnection *WSConnection) Close() {
	...
	if !wsConnection.isClosed {
		...
		//同时room行为
		DebugW("WSConnection Close", "room", wsConnection.rooms)
		if len(wsConnection.rooms) == 1 {
			go func() {
				for room, _ := range wsConnection.rooms {
					RoomBehavior(room, true)
				}
			}()
		}
	}
...
// 检查心跳（不需要太频繁）
func (wsConnection *WSConnection) IsAlive() (alive bool) {
	var (
		now = time.Now()
	)
